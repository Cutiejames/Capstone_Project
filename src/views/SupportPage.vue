...
const mapMessages = (data) =>
  data.map((msg) => ({
    ...
    text: msg.message,
    date: msg.created_at,
    sender: msg.role === "admin" ? "Admin" : "User",
    name: `${msg.f_name || ""} ${msg.m_name || ""} ${msg.l_name || ""}`
      .replace(/\s+/g, " ")
      .trim()
  }));
...
const filteredTickets = computed(() => {
  ...
  return tickets.value.filter((t) => t.status === filter.value || t.ticket_status === filter.value);
});
...
const selectTicket = async (ticket) => {
  ...
  try {
    ...
    const raw = Array.isArray(res.data.messages) ? res.data.messages : res.data.data || [];
...
const sendMessage = async () => {
  ...
  try {
    ...
    await axios.patch(
      `http://localhost:3001/tickets/${selectedTicket.value.ticket_id}`,
      { status: "Answered" },
      { headers: { Authorization: `Bearer ${token}` } }
    );
